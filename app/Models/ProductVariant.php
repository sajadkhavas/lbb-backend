...
class ProductVariant extends Model
{
    ...
    public function mediaAssets(): HasMany { return $this->hasMany(ProductMediaAsset::class, 'variant_id'); }
    public function inventoryReservations(): HasMany { return $this->hasMany(InventoryReservation::class, 'variant_id'); }
    public function inventoryLedgerEntries(): HasMany { return $this->hasMany(InventoryLedgerEntry::class, 'variant_id')->orderBy('id'); }
    ...
    public function getCurrentPriceTomanAttribute(): int { return $this->hasValidSalePrice() ? (int) $this->sale_price_toman : (int) $this->regular_price_toman; }
    public function getPreviousPriceTomanAttribute(): ?int { return $this->hasValidSalePrice() ? (int) $this->regular_price_toman : null; }
    public function getStockOnHandAttribute(): int { return (int) $this->stock_quantity; }
    public function getReservedQuantityAttribute(): int
    {
        if (array_key_exists('active_reserved_quantity', $this->attributes)) { return (int) ($this->attributes['active_reserved_quantity'] ?? 0); }
        return (int) $this->inventoryReservations()->active()->sum('quantity');
    }
    public function getAvailableStockQuantityAttribute(): int { return max(0, $this->stock_on_hand - $this->reserved_quantity); }
    public function getAvailableQuantityAttribute(): int { return $this->available_stock_quantity; }
    public function getAvailableAttribute(): bool { return $this->is_sellable && $this->available_quantity > 0; }
    public function getIsSellableAttribute(): bool
    {
        if (! $this->is_active || $this->color_id === null || $this->size_id === null || blank($this->sku)) { return false; }
        if ($this->relationLoaded('color') && $this->color?->is_active !== true) { return false; }
        if ($this->relationLoaded('size') && $this->size?->is_active !== true) { return false; }
        return Color::query()->whereKey($this->color_id)->active()->exists() && Size::query()->whereKey($this->size_id)->active()->exists();
    }
    public function getLowStockAttribute(): bool { return $this->available_quantity > 0 && $this->available_quantity <= $this->low_stock_threshold; }
    ...
    {
        if ($variant->regular_price_toman < 1) { throw new InvalidArgumentException('قیمت عادی Variant باید بیشتر از صفر باشد.'); }
    ...
    private static function validatePublishedIdentity(self $variant): void
    {
        ...
        if ($product?->publication_status !== PublicationStatus::Published) { return; }
