...
class SystemApiTest extends TestCase
{
    ...
    public function test_meta_reports_commerce_operations_without_claiming_backend_completion(): void
    {
        $this->getJson('/api/system/meta')
            ->assertOk()
            ->assertJsonPath('data.brand.nameEn', 'LBB')
            ->assertJsonPath('data.contractVersion', '2026-08-08-f14-be-e')
            ->assertJsonPath('data.backendComplete', false)
            ->assertJsonPath('data.openApiUrl', '/api/system/openapi');
    }

    public function test_contract_endpoint_reports_be_e_readiness_but_keeps_backend_freeze_closed(): void
    {
